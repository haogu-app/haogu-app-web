...
export function EditRecordModal({ record, onClose, onDeleted, onSaved }: EditRecordModalProps) {
  ...
  const handleUpdate = async () => {
    if (!record._dbId) { setError('無法更新：缺少紀錄 ID'); return; }
    setLoading(true);
    setError('');
    ...
    const { error: dbError } = await supabase
      .from('care_records')
      .update({ original_message: summary, display_message: summary, record_summary: summary, confirmed: true })
      .eq('id', record._dbId);

    setLoading(false);
    if (dbError) { setError('更新失敗，請稍後再試'); return; }
    if (onSaved && record._dbId) onSaved(record._dbId, buildSummary());
    else onClose();
  };
        ...
        <div className="space-y-5">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={subject} onChange={(e) => setSubject(e.target.value)} placeholder="阿嬤"
          ...
          <div>
            ...
            <input type="text" value={detail} onChange={(e) => setDetail(e.target.value)} placeholder="輸入備註內容"
              className="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/20" />
          </div>
